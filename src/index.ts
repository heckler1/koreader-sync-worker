...
async function getUser(kv: KVNamespace, username: string): Promise<User | null> {
	return kv.get<User>(`user:${username}`, 'json');
}

async function addUser(kv: KVNamespace, username: string, userkey: string): Promise<boolean> {
...
async function getPosition(
	...
	const doc = await kv.get<DocumentPosition>(`doc:${username}:${document}`, 'json');
	if (doc !== null) {
		return { ...doc, document };
	}
	return {};
...
async function updatePosition(
	...
	username: string,
	document: string,
	position: { percentage?: number; progress?: string; device?: string; device_id?: string },
): Promise<number> {
...
async function handleRegister(kv: KVNamespace, request: Request): Promise<Response> {
...
async function handleAuthorize(kv: KVNamespace, request: Request): Promise<Response> {
...
async function handleUpdateProgress(kv: KVNamespace, request: Request): Promise<Response> {
	const user = await authorizeRequest(kv, request);
